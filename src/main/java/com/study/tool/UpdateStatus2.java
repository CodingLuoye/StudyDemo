...
public class UpdateStatus2 {
    public static void main(String[] args) throws IOException {
        FileWriter writer=new FileWriter("D:\\交付\\44.log");
        FileReader fr=new FileReader("D:\\交付\\info (1).log");
        BufferedReader br=new BufferedReader(fr);
        String line="";
        String[] arrs=null;
        while ((line=br.readLine())!=null) {
            if(line.contains("收到一网通收款回调接口通知：")) {
                arrs = line.split("收到一网通收款回调接口通知：");
                String cmb = arrs[1];
                CmbIncomingCallBackDto cmbSerial = JSON.parseObject(cmb, CmbIncomingCallBackDto.class);
